...
if ( 'orderrefund' == $action) {
	need_auth('admin');
	$order = Table::Fetch('order', $id);//订单信息
	...
	/*退金币*/
	ZGoldCoinFlow::CreateFromRefund($order);
	/* coupons */
...
elseif ( 'orderremove' == $action) {
...
else if ('bookpay' == $action) {		//后台支付订金,从用户账户余额支付
	need_auth('order');
	$order = Table::Fetch('order', $id);
	if ( $order['service'] == 'bookgold') {  
		if ( $order['state'] == 'halfpay')
			json("本订单已经支付订金", 'alert');
		else{
			$result = ZOrder::ManagePayBook($order);
			if( $result  == 1)
			{
				Session::Set('notice', "订单{$order['id']}支付订金成功！");
				json(null, 'refresh');
			}	
			else if ($result == 0)
				json("用户账户余额不足以支付订金", 'alert');
			else
				json("本订单不需要支付订金", 'alert');
		}		
	}
	else
		json("本订单不需要支付订金", 'alert');	
}
else if ( 'orderpay' == $action || 'ordercash' == $action ) {	//后台支付，从用户账户余额支付
	need_auth('order');
	$order = Table::Fetch('order', $id);
	if ( $order['service'] == 'bookgold'){		//如果是在已经支付了预定金的情况下，再用现金支付剩余的费用
		if ($order['state'] == 'halfpay'){
			if (ZOrder::CashBook($order) == 1)
			{
				$user = Table::Fetch('user', $order['user_id']);
				Session::Set('notice', "订单余款支付成功，购买用户：{$user['email']}");
				json(null, 'refresh');
			}
			else
				json('用户余额不足', 'alert');
		...
		{
			json("本订单尚未支付预订金", 'alert');
		}
		return;
	}	
	if( ZOrder::CashIt($order) != 0)
	{
		$user = Table::Fetch('user', $order['user_id']);
	...
	$order = Table::Fetch('order', $id);
	$extrabuy = DB::LimitQuery('extrabuy', array('condition'=>array('orderid'=>$id)));
	$user = Table::Fetch('user', $order['user_id']);
	$team = Table::Fetch('team', $order['team_id']);
	...
	{
		$goldcoin = min(
					array(intval($user['goldcoin']), 
						  intval($team['goldcoin']) * (intval($order['adult_num']) + intval($order['child_num']))
						)
					);
		if ($order['service'] == 'bookgold')
		{
			if (intval($user['money']) + $goldcoin - intval($order['bookgold']) >= 0)
				$couldpay = true;
			else
			{
				$couldpay = false;
				$needcharge = intval($order['bookgold']) - intval($user['money']) - $goldcoin;
			}
		}
		else
		{
			if (intval($user['money']) + $goldcoin - intval($order['origin']) >= 0)
				$couldpay = true;
			else
			{
				$couldpay = false;
				$needcharge = intval($order['origin']) - intval($user['money']) - $goldcoin;
			}
		}
	}
	else if ($order['state'] == 'halfpay')
	{
		if (intval($user['money']) + intval($order['bookgold']) - intval($order['origin']) >= 0)
			$couldpay = true;
		else
		{
			$couldpay = false;
			$needcharge = intval($order['origin'] - $user['money']) - intval($order['bookgold']);
		}
	}
	if (!isset($needcharge) || $needcharge < 0)
		$needcharge = 0;
...
elseif ( 'categoryedit' == $action ) {
	...
	if ($zone[0] =='city'){
		$html=render('manage_ajax_dialog_city');
	}else if ($zone[0] == 'express') {
...
elseif ( 'called' == $action ) {
	need_auth('order');
	$order = Table::Fetch('order', $id);
	$toggle=($order['called'] == 'N')?'Y':'N';
	Table::UpdateCache('order',$id,array('called'=>$toggle));
	$alert=($toggle=='N')?'未联系':'联系过';
	json( array(	
		array('亲，已经标记为'.$alert, 'alert'),
		array('data'=>'null', 'type' => 'refresh',),
			   ), 'mix');		
}elseif ( 'dealed' == $action ) {
	need_auth('order');
	$order = Table::Fetch('order', $id);
	$toggle=($order['dealed'] == 'N')?'Y':'N';
	Table::UpdateCache('order',$id,array('dealed'=>$toggle));
	$alert=($toggle=='N')?'未处理':'处理过';	
	json( array(
				array('亲，已经标记为'.$alert, 'alert'),
				array('data'=>'null', 'type' => 'refresh',),
			   ), 'mix');
...
elseif ( 'memo' == $action ) {
	need_auth('order');
	$order = Table::Fetch('order', $id);
	$html = render('manage_ajax_dialog_memo');
	json($html, 'dialog');
}
elseif ( 'editmemo' == $action ) {
	need_auth('order');
	$memo=$_GET['t'];
	$id=$_GET['id'];
	DB::Update('order',$id,array('memo'=>$memo));
	json( array(
