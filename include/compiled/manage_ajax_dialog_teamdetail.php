<div id="order-pay-dialog" class="order-pay-dialog-c" style="width:580px;">
	...
	<?php if($team['needline']){?>
		<tr><th colspan="2"><hr/></th></td>
		<tr><td colspan="2">
			<?php if($team['noticesubscribe']){?><button style="padding:0;" id="dialog_subscribe_button_id" onclick="if(confirm('发送邮件过程中，请耐心等待，同意吗？')){this.disabled=true;return X.misc.noticenext(<?php echo $team['id']; ?>,0);}">邮件订阅&nbsp;(<span id="dialog_subscribe_count_id">0</span>/<?php echo $subcount; ?>)</button>&nbsp;<button onclick="window.location.href='/team/preview.php?id=<?php echo $team['id']; ?>';">预览</button><?php }?>
