...
function current_biz() {
	...
	return current_biz_link($l, $a);
}

function current_biz_link($link, $links, $span=false) {
	$html = '';
	$span = $span ? '<span></span>' : '';
	foreach($links AS $l=>$n) {
		if (trim($l,'/')==trim($link,'/')) {
			$html .= "<div class=\"hover\"><a href=\"{$l}\">{$n}</a>{$span}</div>";
		}
		else $html .= "<div class=\"nor\"><a href=\"{$l}\" class=\"p\">{$n}</a>{$span}</div>";
	}
	return $html;
}
...
function current_link($link, $links, $span=false) {
	...
	foreach($links AS $l=>$n) {
		if (trim($l,'/')==trim($link,'/')) {
			$html .= "<li class=\"hov\"><a href=\"{$l}\">{$n}</a>{$span}</li>";
		}
		else $html .= "<li><a href=\"{$l}\" class=\"p\">{$n}</a>{$span}</li>";
	}
	return $html;
}

function hotcity_link($links) {
	$html = '';
	foreach($links AS $l=>$n) {
			$html .= "<a href=\"{$l}\" class=\"ccity\">{$n}</a>";
	}
	return $html;
}
...
function mcurrent_market($selector=null) {
	$a = array(
		...
		'/manage/market/microblog.php' => '新浪微博',
...
function mcurrent_system($selector=null) {
	$a = array(
		...
		'/manage/system/grabhotel.php'=>'爬取酒店',
		'/manage/system/page.php' => '页面',
		'/manage/system/hotcities.php' => '热门城市',
		'/manage/system/cache.php' => '缓存',
		'/manage/system/template.php' => '模板',
		'/manage/system/flushindex.php' => '刷新倒排索引',
		'/manage/system/echoseo.php' => '显示seo信息',
	);
