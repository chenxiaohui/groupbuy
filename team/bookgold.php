...
{	
	...
	$table->bookgold = $table->adult_num * $team['goldbook'];
	...
	$insert = array(
		...
		'quantity', 'create_time', 'remark', 'condbuy', 'traveltime', 'service', 'bookgold'
	);
	
	$flag = $table->update($insert);
	if ($flag)
		redirect("/order/bookfundcheck.php?id={$table->id}");
}
...
include template('team_bookgold');